...
def test__connection_defaults():
    ...
    config_option_mock = MagicMock(side_effect=config_get)
    postgres_opts = {
...
def test__runpsql_with_nested_timeout():
    cmd_run_mock = MagicMock()
    postgres_opts = {
        "config.get": configmod.get,
        "config.option": configmod.option,
    ...
    with patch.dict(postgres.__salt__, postgres_opts):
        with patch.dict(
            configmod.__opts__,
            {"postgres": {"timeout": 60, "pass": None}, "postgres.pass": None},
        ):
            postgres._run_psql("fakecmd", runas="saltuser")
            cmd_run_mock.assert_called_with("fakecmd", timeout=60, **kwargs)
        with patch.dict(
...
def test__run_initdb_with_nested_timeout():
    cmd_run_mock = MagicMock(return_value={})
    postgres_opts = {
        "config.get": configmod.get,
        "config.option": configmod.option,
    ...
    cmd_str = (
        "/fake/path --pgdata=fakename --username=saltuser "
    ...
    with patch.dict(postgres.__salt__, postgres_opts):
        with patch.object(postgres, "_find_pg_binary", return_value="/fake/path"):
            with patch.dict(
                configmod.__opts__,
                {"postgres": {"timeout": 60, "pass": None}},
            ):
                postgres._run_initdb("fakename", runas="saltuser")

                cmd_run_mock.assert_called_with(
                    cmd_str,
                    timeout=60,
                    **kwargs,
                )
